...
5b. **For ANY HVAC work (HVAC coordinator, thermostats, presets, borrows/excursions, nudges/AC ramp, arrester, HVAC occupancy, zones) — `docs/Coordinator/HVAC_ARCHITECTURE_STATE_OF_PLAY.md` is MANDATORY READING, COMPLETELY, BEFORE planning, building, reviewing or diagnosing anything.** Operator 2026-09-26: "every agent used in the rest of the arc reads first completely before doing a damned thing". It supersedes the older HVAC design docs/manual/READMEs/memory for the areas it covers (code at the cited file:line wins over the doc — fix the doc in the same turn). Its §10 CORRECTIONS LEDGER lists claims that were WRONG — never re-assert them. Every HVAC ship or correction updates it in the same commit.
