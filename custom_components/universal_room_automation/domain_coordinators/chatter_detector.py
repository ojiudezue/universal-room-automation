...
def _classify(entity_id: str, kind: str, provider: Optional[str],
              integration: Optional[str]) -> Tuple[bool, Optional[str]]:
...
class ChatterDetector:
    ...
    def _resolve_provider(self, entity_id: str) -> Tuple[Optional[str], Optional[str]]:
        """Return ``(provider, integration)`` best-effort from entity registry.

        Provider = URA capability tag if the sensor_capability layer knows
        it; else None (classifier falls back to entity_id substring).
        Integration = HA entity-registry platform (used by the
        camera-family guard).
        """
        ...
        try:
            ...
            provider = None
        # Entity-registry platform (integration domain).
        try:
    ...
    def async_register_listeners(self) -> None:
        ...
        for eid, kind in self._iter_configured_entities():
            provider, integration = self._resolve_provider(eid)
            in_scope, provider_used = _classify(eid, kind, provider, integration)
            if not in_scope:
                continue
            t_floor = _t_floor_for(kind, provider_used or "")
            if t_floor <= 0.0:
    ...
    def _on_edge(self, event: Event) -> None:
        ...
        try:
            ...
            # Boot-settle gate: sample edges but do NOT score them until
            # boot settle has released (prevents restart-flurry false-fires).
            boot_settled = True
            try:
                ...
                if interval < t_floor:
                    ...
                    if (
                        ...
                        _LOGGER.info(
                            ...
                            "(%d sub-%.1fs impossibility events in %ds window)",
                            self._room_name, entity_id, len(sf),
                            t_floor, int(CHATTER_OBSERVATION_WINDOW_S),
                        )
