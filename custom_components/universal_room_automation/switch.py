...
async def async_setup_entry(
    ...
    if entry_type == ENTRY_TYPE_COORDINATOR_MANAGER:
        ...
        async_add_entities([
            ...
            # HVAC-GOVERNED-EXCURSION-1 D2 §4.7: Excursion Primitive kill
            # switch. Default ON. BEGIN-ONLY — persisted rows continue to
            # fire return_excursion regardless of switch state.
            HVACExcursionPrimitiveEnabledSwitch(hass, entry),
...
class HVACExcursionPrimitiveEnabledSwitch(SwitchEntity, RestoreEntity):
    """HVAC-GOVERNED-EXCURSION-1 §4.7 — Excursion Primitive kill switch.

    Default ON. BEGIN-ONLY semantics: turning OFF causes
    ``hvac_excursion.begin_excursion`` to return None (no state row, no
    lease, no suppress, no wire write), but every already-persisted
    ``hvac_excursion_state`` row STILL fires ``return_excursion`` at its
    timer callback and at the boot audit. Flipping OFF mid-flight does
    NOT strand an in-progress excursion.

    Pattern mirrors ``HvacOffphaseHonestyEnabledSwitch`` (constructor
    kwarg seed on the coordinator + property/setter pair + RestoreEntity
    + deferred-value handoff via SIGNAL_HVAC_COORDINATOR_READY).

    Entity: switch.ura_hvac_coordinator_excursion_primitive_enabled
    """
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        self._attr_unique_id = (
            f"{DOMAIN}_hvac_coordinator_excursion_primitive_enabled"
        )
        self._attr_name = "Excursion Primitive"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, "hvac_coordinator")},
            name="URA: HVAC Coordinator",
            manufacturer="Universal Room Automation",
    ...
    def is_on(self) -> bool:
        ...
        if hvac is None:
            return True
        try:
            return bool(hvac.excursion_primitive_enabled)
        except Exception:  # noqa: BLE001
            return True

    async def _set(self, target: bool) -> None:
        hvac = self._get_hvac()
        if hvac is not None:
            try:
                hvac.excursion_primitive_enabled = target
            except Exception:  # noqa: BLE001
                pass
            self._deferred_value = None
            self.async_write_ha_state()
            if target:
                _LOGGER.info(
                    "HVAC-GOVERNED-EXCURSION-1: Excursion Primitive enabled"
                )
            else:
                _LOGGER.warning(
                    "HVAC-GOVERNED-EXCURSION-1: Excursion Primitive DISABLED "
                    "(begin-only) - new excursions will NOT be started; "
                    "in-flight excursions continue to their return"
                )
        try:
            self.hass.config_entries.async_update_entry(
                self._entry,
                options={
                    **self._entry.options,
                    "excursion_primitive_enabled": target,
                },
            )
        except Exception:  # noqa: BLE001
            _LOGGER.debug(
                "HVACExcursionPrimitive options-writeback failed",
                exc_info=True,
            )
    ...
    def _handle_hvac_ready(self) -> None:
        ...
        if hvac is None:
            return
        try:
            hvac.excursion_primitive_enabled = self._deferred_value
        except Exception:  # noqa: BLE001
            pass
        self._deferred_value = None
        self.async_write_ha_state()
