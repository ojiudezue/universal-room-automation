...
_Generated: 2026-09-15T22:57:37-05:00_ - _Data commit: `dc0db6a9a9e8`_ - _last_reconciled: 2026-09-15_
...
thread: **hvac** - status: **investigating** - approval: **explicit**
_created 2026-08-20 14:15 · updated 2026-09-15 · refined ×3_
- **Problem / Solution:**
...
- **Next:** MEASURE FIRST — residual B gates residual A, so nothing is built this pass. Tonight (pairs with the operator's 20:00 restart window on AGGREGATION-ENTITY-ADDED-THREAD-SAFETY-1, then again after local midnight): re-run the metric_baseline...
...
### `AGGREGATION-ENTITY-ADDED-THREAD-SAFETY-1` - AggregationEntity.async_added_to_hass off-loop async_create_task (house-wide, HA 2027 deprecation) — _#2 · WSJF 2.0 · v5 tc3 u2 /e5 ⚠_
...
### `HVAC-GUEST-AS-ZONE-PERSON-1` - A guest in an occupied guest room should count as that zone's "person" — today a zone with no assigned residents loses three protections entirely — _#2 · WSJF 2.0 · v5 tc3 u2 /e5 ⚠_
...
### `EC-SOC-LADDER-XVALIDATE-1` - No cross-field validation on the EC SOC ladder — inverted operator sliders can flip a gate polarity and oscillate EV pause/resume; the parked fix's trigger has now fired — _#4 · WSJF 2.0 · v5 tc3 u2 /e5 ⚠_
...
- **Next:** Tier 1. Fix the two EVSE onset call sites to pass a real entity_id (the bay's charger switch) or omit the field when only a bay slug is known. THEN add the guard at the shared producer — activity_logger.py:114 should refuse to put a valu...
...
### `ARRESTER-LEDGER-INVISIBLE-1` - The HVAC override arrester keeps its ledger in INFO log lines, on a log that only records WARNING and above — so every arrest it makes is invisible after the fact — _#6 · WSJF 2.0 · v5 tc3 u2 /e5 ⚠_
...
- **Next:** Scope as additive observability (Tier 1-2, no behaviour change): emit an activity_logger row at the arrester decision sites, REUSING the hvac_fans.py:1058 pattern, with an action vocabulary that distinguishes detected / reverted / suppre...
...
### `UNLOAD-SYMMETRY-TASK-HYGIENE-1` - Setup/unload symmetry + tracked background tasks (tech-debt hardening) — _#9 · WSJF 2.0 · v5 tc3 u2 /e5 ⚠_
...
### `HVAC-MANUAL-PRESET-CONTRACT-1` - Design spec says control the thermostats via PRESETS, never raw manual setpoints — reality is zones sitting in manual for hours; do the sanctioned excursions return? — _#11 · WSJF 1.5 · v5 tc3 u4 /e8 ⚠_
thread: **hvac** - status: **planned** - approval: **unreviewed**
...
### `S14-CEILING-NEEDS-AN-ENDING-1` - S14 off-phase ceiling hold has no exit and blocks its own — give it an ending (operator chose option (a) 2026-08-21), preferably by making it a borrow kind — _#12 · WSJF 1.2 · v5 tc3 u2 /e8 ⚠_
...
### `INTEGRATION-CAMERA-DISCOVER-STALE-1` - Adding/removing a camera while its config-save reload is suppressed leaves the shared camera→area map stale — new camera never extends room occupancy until restart — _#22 · WSJF 1.2 · v5 tc3 u2 /e8 ⚠_
