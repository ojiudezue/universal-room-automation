...
### `EGRESS-CAMERA-LIST-STALE-5-NOT-3-1` - Live egress_cameras is FIVE (adds garage_a, garage_b) but the const and its derived list assume THREE — _#1 · WSJF 4.7 · v7 tc5 u2 /e3_
...
- **Next:** ANSWER whether garage_a and garage_b should be treated as EGRESS cameras for perimeter alerting (they are already configured as such). If yes -> place them in the ring relative to back_yard, re-derive EXTERIOR_TRACK_EGRESS_ADJACENT_CAMER...
...
- **Next:** Fold into the seam-file cycle if it is built (same pattern, same critique C3), or ship standalone as a Tier-1: read the key at __init__.py:3449 with DEFAULT_TOU_RATE_FILE as the fallback, add the options-flow field, and add a test that s...
...
### `CAMERA-SEAM-RING-CONTRADICTION-1` - The written ring contradicts the operator's own seam notes — pool_equipment and the Reolink/PT-Ultra slot look swapped — _#1 · WSJF 18.0 · v8 tc6 u4 /e1_
...
### `CAMERA-PTULTRA-NOT-IN-PERIMETER-1` - 'Madrone PT Ultra' is a real camera with person detection that is NOT in URA's perimeter list — _#2 · WSJF 7.0 · v7 tc5 u2 /e2_
thread: **perimeter** - status: **waiting_operator**
_created 2026-09-13 23:05 · initial_
- **Problem / Solution:**
...
- **Next:** ANSWER: when you wrote "Madrone PT Ultra/Reolink hub" in the ring, did you mean (a) the study-B porch camera URA already watches (camera.reolinkstudybporchptz), or (b) the separate Madrone PT Ultra camera? If (b), it is an exterior camer...
- **Tags:** no-fabrication-verify
...
### `CAMERA-SEAM-VALIDATION-1` - Exterior camera seams (handoff points) listed for operator validation — 12 cameras, 27 seams — _#3 · WSJF 5.5 · v6 tc3 u2 /e2_
thread: **perimeter** - status: **waiting_operator**
