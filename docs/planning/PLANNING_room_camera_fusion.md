...
## 2026-08-01 — UPSTREAM FINDING (homelab agent, operator-relayed): Frigate 1 + Frigate 2 share MQTT topic_prefix `frigate`

Both Frigate hosts publish stats/events to IDENTICAL topics; HA's two Frigate config entries subscribe to the same merged stream. Consequences URA-side (do not fix here — the prefix change is a homelab-side task with HA-side coordination, filed there):

1. **Re-frames the playroom phantom incidents (#1-#3):** what presented as "static-object detection FPs surviving threshold hardening" may be (partly or wholly) CROSS-HOST TOPIC BLEED — host B's payloads (or MQTT retained messages) landing on host A's entities. Evidence consistent with bleed: the rapid unavailable⇄value flapping on person_count entities; and during the 2026-08-01 incident, reloading Frigate 1 cleared `playroom_person_count` while `_2` still held 1 (retained/cross-published payload re-delivered to the other entry).
...
## Amendment 2026-08-01 (operator direction): Frigate 2 onboarding, corroboration semantics, MAC derivation

1. **Frigate 2 onboards across the board.** F2 is the strategic host (more compute, deeper features — genai/Ollama, OpenVINO×3); F1 retires eventually. The resolver and all consumers must treat F2 as a first-class source from this cycle, not an appendix. Design for F1's eventual removal: nothing may hard-key on the F1 entry.
2. **F1↔F2 corroboration UNLOCKED behind a defined stability gate** (supersedes the open-ended "until the split proves stable"): the two NVRs are on different hosts, so post-split their agreement is genuinely independent evidence. GATE (all three, measured over 72h from the 2026-08-01 ~13:30 CDT prefix split; earliest open ~2026-08-04):
   (a) zero MQTT session evictions for client_ids frigate-f1/frigate-f2 (broker-side; the pre-fix signature was 8+5/24h);
   (b) zero unavailable⇄value flapping bursts on F1/F2 person_count entities (the pre-split signature);
   (c) no retained-message ghosts — person_counts return to 0 after event end on both entries.
   Verification is a one-shot recorder/broker read, not a soak ritual; any failure resets the 72h clock.
3. **Face detection semantics:** available+ENABLED = usable corroborator (configured implies intent); DISABLED = AMBIGUOUS capability (needs edge/NVR support — treat as unknown, NOT negative evidence, and do not auto-enable face per the standing person-yes/face-opt-in rule); absent = absent.
4. **Single-integration (consumer-grade) cameras are a supported case, not a failure**: resolve to ≥1 source and participate normally; multi-integration is the corroboration happy path, single-source keeps current single-source semantics.
5. **Amcrest**: multiple amcrest-family integrations exist in this install — the resolver's correlation must handle same-vendor multi-integration exposure (same class as Frigate×2).
6. **NEW correlation rung — network-inventory join (operator suggestion):** most cameras are on the network; when a camera integration's device lacks a usable MAC in `device.connections`, derive it by joining IP/hostname against the UniFi Network client inventory (MAC+IP+hostname authoritative there). Ladder becomes: same-device → device MAC → identifiers → NETWORK-INVENTORY JOIN (IP/hostname→MAC) → name-stem → operator declaration. The join is a build-time/refresh-time lookup (not per-tick); its failure degrades to the next rung. D0 probe follow-up: include the UniFi client table join in the pairing report where MACs were missing.

## Amendment 2026-08-01 (operator): FIFTH reviewer + corroboration-count doctrine

**Review protocol for this cycle is Tier 3 + 1: FIVE reviews.** Reviews A-D per Tier-3 (local correctness / integration / test-authority-mutation / adversarial completeness) PLUS **Review E — external-expert framing**: a reviewer acting as an outside specialist in camera cross-corroboration who FIRST does external research (web) on the state of the art — multi-detector fusion practice, Frigate/Protect/Reolink detector characteristics and failure modes, sensor-fusion literature (voting vs weighted agreement vs Dempster-Shafer-style evidence handling), known pitfalls of correlated detectors — and THEN reviews the resolver + fusion design against that grounding. Operator: "We have to get it right."

**Corroboration-count doctrine (operator question adjudicated):** independence beats count. Heterogeneous sources (different detector models/processing paths: Frigate-OpenVINO vs Protect-proprietary vs Reolink-edge) each add real evidence; same-family sources ingesting the same stream add ~zero and inflate false confidence (correlated errors agree with each other). Rules for the fusion layer:
- Effective corroborator cap: ~3 heterogeneous source FAMILIES per camera; additional sources beyond that fold into their family's single vote rather than stacking.
- Same-family collapse is structural (already in D2 for Frigate×2 pre-gate); post-gate F1↔F2 count as semi-independent (same model family, different hosts) — HALF-weight family, not two votes.
- Capability DIVERSITY is the value signal (operator insight): platforms exposing slightly different capabilities (person vs face vs count vs zones) corroborate across MODES, which is stronger than N identical person-bits. The capability map should preferentially pair diverse modes when scoring agreement.
- Review E explicitly evaluates these rules against the literature and may propose replacements with citations.
