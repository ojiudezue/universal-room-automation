...
2. **CROSS-integration `device_id`/MAC/model joins — no reliable key** (canonical: `docs/architecture/DEVICE_TREE.md`). De-dup ACROSS integrations = operator-declared only. **AND `device_id` is NOT a reliable appliance boundary within v1a either** (review A1, 2026-09-14): a ThinQ washer = 1 device / many entities (→ merge), but a 2-channel Shelly 2PM = 1 device / 2 appliances (→ must NOT merge). Auto-merging by `device_id` dropped 27 live entities. **v1a decision: NO auto-merge — source-3 emits one `other` record per unique unclaimed `entity_id` (dedup by entity_id only), and ALL grouping is operator-declared** (v1b). This honors no-drop and matches the operator's declared-grouping model; device_id-assisted grouping *suggestions* may come in v1b onboarding, never as a silent v1a merge. (The `camera_census.py:589-628` pattern dedups homogeneous `camera.*` duplicates — not heterogeneous roles on one device — so it does not transfer here.)
