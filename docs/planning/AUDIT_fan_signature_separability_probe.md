...
- **Enable the disabled LD2410 energy entities** (all `disabled_by: integration`): `sensor.hlk_ld2410_07cb_still_energy`, `sensor.hlk_ld2410_3616_still_energy`, `sensor.hlk_ld2410_aff4_still_energy`, `sensor.hlk_ld2410_bdbc_still_energy` and matching `*_detection_distance` (`ld2410`/`ld2410_ble` platforms) — if any map to fan-equipped rooms, they add the still_energy corroborator there.
  - **2026-08-01 attempted — BLOCKED, operator decision required.** The entity-level `disabled_by: integration` is not the real gate: all four `ld2410_ble` config entries are `disabled_by: user` (entries `01KJVP2F…AFF4`, `01KNM617…BDBC`, `01KNM63C…07CB`, `01KRKTZ8…3616`, all `not_loaded`), and the paired `ld2410` entry is likewise disabled. The operator disabled these radars wholesale — consistent with the HAOS BLE socket budget constraint. Re-enabling means reconnecting four BLE devices, a budget call, not a config toggle. Until then the still_energy corroborator has NO live data path; the transition-coincidence gate (primary mechanism, no new hardware) is unaffected.
