...
- **Investigated 2026-07-31 (root cause found, fix deferred):** NOT the v5.16.2 boot-ordering race (that fix is intact — pre-start writes buffer losslessly). DB row-timeline shows the worker runs fine from first write (~10 min pre-STARTED), then ALL write streams stall for ~6.5 min exactly in the HA STARTED window (03:05→03:11Z on the 07-30 boot; the four 35s-timeout errors at 03:08Z are the optimizer/census/energy/house-state writers caught mid-stall — their rows ARE lost: on ready-timeout the caller raises and the worker later no-ops the item). Boot-window write VOLUME is trivial (97 rows/8 min), so this is not a flood. Mechanism: STARTED-time thundering herd — event-loop congestion severe enough that entity updates take 1.5s+ (system_log warnings) starves the worker task and/or prolongs the SPAN scope-repass `BEGIN IMMEDIATE` transaction, blocking the serial writer >35s. Load added since 07-17 (onset era): v5.21.0 shadow-eval turned on `optimization_findings` (~1/day → ~2,400-2,800/day — but batched 1 write/cycle with a boot-storm skip, so a steady-state non-issue), plus a month of new STARTED-time work (watchdog, digest, savings, Residence aggregates); DB now 1.05 GB (+110 MB in ~2 wks post-VACUUM). Nothing regressed in database.py itself.
- **Fix shape (revised):** make the 35s ready-timeout lossless (leave the item queued, WARN, complete late) rather than raise-and-drop; and/or a STARTED-window grace multiplier on the caller timeout. Do NOT chase write-volume — it's not the cause.
