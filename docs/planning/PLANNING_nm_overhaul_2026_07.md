...
1b. **Post-upgrade live-channel test authorization (operator, 2026-07-20):** transport pipes pre-validated this date (BlueBubbles iMessage + WhatsApp to +14258299520, notify.madronehapushover — iMessage + Pushover delivery CONFIRMED by operator echo 2026-07-20; WhatsApp send succeeded service-side, delivery unconfirmed — re-verify before Cycle C relies on it). Filling these targets is authorized ONLY as part of post-Cycle-C autonomous validation: a small, clearly-labeled test send per channel (iMessage first — operator primary), then the live handover. Not before.
