...
| S14 | `hvac.py` D5 else-limb (~:1604) | `emit_set_temperature` | HVAC-PRESET-FLAP-1: duty off-phase honest setpoint write (holds `home_target_high + COMFORT_OFFPHASE_OFFSET_F` instead of forcing `preset=away`) in occupied zones with `runtime_exceeded` | **DEFER** *(reason `runtime_exceeded_offphase`)* | Introduced by HVAC-PRESET-FLAP-1 (`docs/planning/PLANNING_preset_flap_offphase_honesty.md`). Site tag `S14_duty_off_phase`. Aligns with S1's DEFER treatment of the `runtime_exceeded` reason — comfort-grace suppresses the honesty write for the same reason it suppresses the S1 forced-away write. Dominance short-circuits (stale_occupancy / vacant_past_grace / within-grace vacancy / kill-switch / shed_active) run BEFORE the helper so those branches keep the pre-cycle preset ladder. |
