...
**Location:** new `ChatterDetector` module at `custom_components/universal_room_automation/domain_coordinators/chatter_detector.py`. Sibling to `sensor_exclusion.py` / `sensor_role.py` / `sensor_capability.py`; module-not-inline chosen for testability (alternative "helper in coordinator.py" REJECTED per plan-review-2 L-LOW-A). Registered as a subscriber on `OccupancySubstrate.subscribe()` for state-change edges.

**Subscribe lifecycle (Bug Class #38 discipline — L-LOW-B fix).** `OccupancySubstrate.subscribe(cb)` returns an unsubscribe callable. `ChatterDetector` stores the returned callable as `self._chatter_unsub` on the detector instance (and the detector instance itself is held on the RoomCoordinator as `self._chatter_detector`, mirroring `self._actuator_reconciler`). The unsubscribe MUST be called from `RoomCoordinator.async_will_remove_from_hass()` (or equivalent config-entry unload / teardown hook — mirror the existing subscription-cleanup pattern used for actuator listeners), pattern:
```
...
```
Add a lifecycle test `test_chatter_detector_unsubscribe_called_on_teardown` that: constructs a RoomCoordinator with a mock substrate, verifies `substrate.subscribe` was called once and returned a spy callable, invokes `async_will_remove_from_hass()`, asserts the spy was invoked exactly once and no callback survives in the substrate's subscriber list. Reviewer B verifies no leaked subscribers survive a room-entry reload (individual room entries CAN reload; CM reload-suppression covers only the parent).
...
- **Verify (subscribe teardown — L-LOW-B):** `test_chatter_detector_unsubscribe_called_on_teardown` asserts `substrate.subscribe`'s returned callable is invoked exactly once on `RoomCoordinator.async_will_remove_from_hass()` and no callback survives a room-entry reload.
...
7. **Subscribe-teardown lifecycle wire test (L-LOW-B):** `test_chatter_detector_unsubscribe_called_on_teardown` — DELETE the `self._chatter_unsub()` call in `ChatterDetector.async_teardown()`; assert the lifecycle test reds. Restore + status-check.
...
| `custom_components/universal_room_automation/domain_coordinators/chatter_detector.py` (new) | `ChatterDetector` (subscribe/teardown + physics criterion + release) | +240 |
| `custom_components/universal_room_automation/coordinator.py` | Migrate 6 consumer sites onto `SensorExclusionSet.is_excluded()`; migrate P22 + STUCK-SENSOR-1 D1 writers onto `promote()` (lock-step with `_dutycycle_excluded_now` per §D1.1); construct `ChatterDetector`, wire `substrate.subscribe(...)`, store `self._chatter_detector` + call `async_teardown()` in `async_will_remove_from_hass`; tick-site promotion/release block + kill-switch helper; PRESERVE `_d2_completed_cleanly` failure-branch semantics unchanged per §D1.1 Reading A (do NOT re-populate `SensorExclusionSet` mirror on the failure branch) | +200 |
...
| `quality/tests/test_chatter_detector.py` (new) | D2 definition-agnostic tests + D4 no-PIR test + M-MED-2 provenance-gate fixture tests + L-LOW-B `test_chatter_detector_unsubscribe_called_on_teardown`; definition-anchored tests added when D0 lands | +250 |
| `quality/tests/test_chatter_release.py` (new) | D3 tests | +140 |
| `quality/tests/test_unavailable_entities_chatter.py` (new) | D5 tests | +80 |
| `quality/tests/test_chatter_wire_in.py` (new) | D6 wire-in + mutation drill (chatter + 6 consumer sites + release-scan failure-branch add-promote + subscribe-teardown) | +240 |
...
- Subscribe teardown → **`self._chatter_unsub` stored on `ChatterDetector`; teardown from `RoomCoordinator.async_will_remove_from_hass()`; lifecycle test `test_chatter_detector_unsubscribe_called_on_teardown`.** Resolved 2026-08-19 per plan-review-2 L-LOW-B.
