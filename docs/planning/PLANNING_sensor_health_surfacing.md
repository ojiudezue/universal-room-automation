...
| Sensor kind → CONF bucket | **REUSED** | `occupancy_substrate._KIND_TO_CONF` at `occupancy_substrate.py:82-86`; `TIER1_KINDS` at `const.py:342`. Chatter detector inverts this map (plus the opener CONF) at setup to build its `_entity_to_kind` lookup. |
...
| Per-entity state-change listener | **REUSED (HA core helper)** | Chatter detector registers its OWN `async_track_state_change_event(hass, <blind-time-gated entity set>, self._on_edge)` at setup. Same core helper URA uses elsewhere (e.g. `coordinator.py:1350`). **`OccupancySubstrate.subscribe()` is NOT used by the chatter detector** — its callback signature is `Callable[[str, str, bool], None]` = `(room, kind, on)` (per-KIND AGGREGATE; `occupancy_substrate.py:704-706`) and cannot deliver the per-entity edges the sub-`T_floor` burst counter needs. Modest duplicate listen vs `coordinator.py:1350` is intentional (different consumer; HA dispatches natively; negligible cost) and buys self-containment + honest Bug Class #38 teardown (the detector owns its own unsub). |
...
- `custom_components/universal_room_automation/domain_coordinators/occupancy_substrate.py` end-to-end (subscribe signature at `:704-706` confirms per-KIND aggregate callback — the reason chatter uses its own HA listener instead).
...
**Location:** new module `custom_components/universal_room_automation/domain_coordinators/sensor_exclusion.py`. Sibling architecture (`sensor_role.py`, `sensor_capability.py`) — easier to test, keeps coordinator lean. (Alternative "helper in coordinator.py" option is REJECTED per plan-review-2 L-LOW-A; do not re-open at build time.)
...
3. **`_d2_completed_cleanly` guard preserved, byte-identically — Reading A (bookkeeping-only preservation).** The mid-detector exception guard at `:2698-2705` preserves STUCK-SENSOR-1's OWN bookkeeping fields (`_dutycycle_excluded_last_tick`, `_dutycycle_excluded_now`) so the next tick's release-scan diff remains honest — this is the B-MED-1 semantics, unchanged. The `SensorExclusionSet` MIRROR is NOT re-populated on the failure branch: because `reset_tick()` cleared the set at tick start and the D1 promotion loop at `:2567-2569` never ran, the shared set contains ONLY the P22 promotions performed earlier in the tick. Therefore on the failure tick the fusion excludes ONLY P22 entities — **byte-identical to shipped v5.75.0 behavior**, which this cycle FORMALIZES and must not change. STEP-EXCLUDE-2 holds strictly on the D2-raise codepath. Builder MUST NOT add a compensating `promote("stuck_dutycycle", e)` (or any re-population of prev-tick D1 entities into `SensorExclusionSet`) in the failure branch — doing so is Reading B (fusion re-engagement), a NEW behavior orthogonal to B-MED-1 and outside this cycle's scope. If field evidence later argues for Reading B (fewer transient vote drops on transient detector faults), it belongs in a separate follow-up cycle with its own named "D2-raise fusion preservation" acceptance test. Reviewer A's `ledger_golden_replay` proves byte-identity on the D2-raise codepath via a required NEW failure-tick fixture: a fixture where `_detect_duty_cycle_stuck` raises mid-loop for one tick, asserting the failure-tick fusion output equals pre-cycle (P22-only) byte-for-byte. This is now a required D1.1 acceptance fixture.
...
Reviewer B owns end-to-end trace verification of this ordering AND the Reading-A failure-branch semantics (no compensating mirror-promote); Reviewer C's per-site mutation drill includes ADDING a compensating `promote("stuck_dutycycle", e)` into the failure branch and asserting the D2-raise byte-identity fixture reds (the forbidden Reading-B mutation); Reviewer D re-enumerates whether any newly-introduced code path could reset tick-state at the wrong point or smuggle in Reading-B via a helper.

### D2 — Chatter client (physics-based; DEFINITION GROUNDED + HAND-CHECKED per D0)

**Location:** new `ChatterDetector` module at `custom_components/universal_room_automation/domain_coordinators/chatter_detector.py`. Sibling to `sensor_exclusion.py` / `sensor_role.py` / `sensor_capability.py`; module-not-inline chosen for testability (alternative "helper in coordinator.py" REJECTED per plan-review-2 L-LOW-A). The detector registers its OWN per-entity HA state-change listener over the room's blind-time-gated entity set; it does NOT consume `OccupancySubstrate.subscribe()`.

**Why not `OccupancySubstrate.subscribe()` (build-time gap corrected 2026-08-19).** The substrate `subscribe()` feed at `occupancy_substrate.py:704-706` has signature `Callable[[str, str, bool], None]` — arguments `(room, kind, on)` — i.e. per-KIND AGGREGATE edges (the kind bucket flipped True/False after the substrate OR-ed all entities of that kind). The chatter detector needs per-ENTITY transition timestamps to count sub-`T_floor` impossibility events on individual entities; the aggregate signal cannot provide that (multiple entities of the same kind collapse into one edge and the offending entity's identity is lost). The plan's prior "no new registration / inherit subscribe / Bug Class #38 discipline inherited" framing was based on the FALSE assumption that `subscribe()` delivered per-entity edges — that framing is VOID; the detector owns its own listener. This is Option 1 (orchestrator-chosen 2026-08-19): self-contained module, owns its lifecycle, makes the Bug-Class-#38 teardown REAL.

**Listener registration + entity set.** In `ChatterDetector.async_setup(...)`:
```
from homeassistant.helpers.event import async_track_state_change_event

self._chatter_unsub = async_track_state_change_event(
    self.hass, list(self._gated_entity_ids), self._on_edge
)
```
The entity set = the room's Tier-1 sensors filtered to `(kind, provider)` tuples in `CHATTER_PROVENANCE_ALLOWLIST` (PIR / mmWave / opener / reed — per §D2 point 1 below). Cameras / AI / aggregates / bed-multistate are excluded at set-construction time (they are also denied by the classifier at scoring time; excluding them from the listener set means the listener never fires on them and the classifier DENY is defense in depth). If the entity set is empty for a room (no blind-time-gated sensors), no listener is registered and `self._chatter_unsub = None`. The modest duplicate-listen vs `coordinator.py:1350` is intentional (different consumer, HA dispatches natively, negligible cost) and self-containment / testability is why.

**`_on_edge(event)` — entity_id → kind derivation (the piece the per-kind feed would have given for free).** The HA event carries `event.data["entity_id"]`, `event.data["new_state"]`, and `event.time_fired`. To feed the provenance classifier and the per-family `T_floor` lookup, `_on_edge` must derive `kind` from `entity_id`. Concrete mechanism (specified so the builder does not guess): **invert `_KIND_TO_CONF` at `occupancy_substrate.py:82-86`** — `{"motion": CONF_MOTION_SENSORS, "mmwave": CONF_MMWAVE_SENSORS, "occupancy": CONF_OCCUPANCY_SENSORS}` — plus the opener CONF (openers are not in `_KIND_TO_CONF`; look up whichever CONF list the RoomCoordinator uses for garage/opener sensors — `CONF_OPENER_SENSORS` or the equivalent under the room's config schema, and add it to the inversion map). Build a one-time `self._entity_to_kind: dict[str, str]` at `async_setup` by walking each of the room's CONF_*_SENSORS lists once and recording `{entity_id: kind}` membership for every entity that survived the `CHATTER_PROVENANCE_ALLOWLIST` gate. `_on_edge` then does:
```
def _on_edge(self, event):
    entity_id = event.data.get("entity_id")
    new_state = event.data.get("new_state")
    ts = event.time_fired
    kind = self._entity_to_kind.get(entity_id)
    if kind is None:
        return  # defensive — shouldn't happen; listener set == inversion domain
    if new_state is None or new_state.state in ("unavailable", "unknown"):
        return  # unavailable bursts are a separate fault class (see algorithm skeleton step 2)
    self._observe_edge(entity_id, kind, ts, new_state.state)
```
`_observe_edge` performs per-entity sub-`T_floor` burst counting (algorithm skeleton steps 3-4 below).

**Subscribe lifecycle (Bug Class #38 discipline — REAL, L-LOW-B fix).** `async_track_state_change_event(...)` returns an unsubscribe callable. `ChatterDetector` stores it as `self._chatter_unsub`. `async_teardown()` calls `self._chatter_unsub()` if set and clears the reference. The detector instance is held on the RoomCoordinator as `self._chatter_detector` mirroring `self._actuator_reconciler`; the teardown is invoked from `RoomCoordinator.async_will_remove_from_hass()` (or equivalent config-entry unload / teardown hook — mirror the existing subscription-cleanup pattern used for actuator listeners):
```
if getattr(self, "_chatter_detector", None) is not None:
    self._chatter_detector.async_teardown()   # calls self._chatter_unsub() if set, then clears the ref
    self._chatter_detector = None
```
Add a lifecycle test `test_chatter_detector_unsubscribe_called_on_teardown` that: constructs a RoomCoordinator with a mock hass whose `async_track_state_change_event` is spied to return a Mock unsub callable; verifies the spy return is stored on the detector after `async_setup`; invokes `async_will_remove_from_hass()`; and asserts the HA unsub Mock was invoked EXACTLY ONCE (i.e. the HA-returned unsubscribe callable — more real than the old subscribe-shaped test which spied a URA-internal callback list). Reviewer B verifies no leaked HA listener survives a room-entry reload (individual room entries CAN reload; CM reload-suppression covers only the parent).
...
1. **Provenance gate (the un-fakeable-property gate).** The criterion applies ONLY to sensors whose
   ...
   **Source of `(kind, provider)`:** the `(kind)` comes from URA's existing `_KIND_TO_CONF` bucket at
   `occupancy_substrate.py:82-86` (same inversion used to build `_entity_to_kind` — see the listener
   section above). The `(provider)` comes from URA's existing capability / provider tagging on the
   entity (same mechanism SENSOR-CAPABILITY-1 uses). Where no explicit provider tag is available, fall
   back to an integration-domain / entity_id regex sub-allow-list committed alongside
...
1. On every HA state-change event delivered to `_on_edge` for entity `e` in room `R` (kind derived via `self._entity_to_kind[e]`):
2. If `new_state.state ∈ {"unavailable", "unknown"}` → do NOT count as a transition (unavailable bursts are a separate fault class).
...
- **Verify (listener teardown — L-LOW-B):** `test_chatter_detector_unsubscribe_called_on_teardown` asserts the HA `async_track_state_change_event`-returned unsubscribe callable is invoked exactly once on `RoomCoordinator.async_will_remove_from_hass()` and no HA listener survives a room-entry reload.
...
1. **Listener-wire test:** monkey-patch `ChatterDetector._on_edge` to record calls; fire 5 HA state-change events on a covered entity via the mock hass; assert 5 calls. DELETE the `async_track_state_change_event(...)` registration in `ChatterDetector.async_setup(...)`; assert the detector never sees edges and the test reds.
...
6. **Release-scan failure-branch wire test (D1.1 — Reading-A anti-inversion) — M2-MED-1 fix.** Two sub-mutations, both driven against a NEW named fixture `test_d2_raise_fusion_byte_identity_reading_a` that constructs a tick where `_detect_duty_cycle_stuck` raises mid-loop and asserts the fusion output for THIS tick equals pre-cycle byte-for-byte (P22-only exclusion, no D1 entities):
   - **6a (forbidden add-promote):** source-mutate the `_d2_completed_cleanly=False` branch at `:2698-2705` to insert a compensating `for e in _prev_excluded: exclusion_set.promote("stuck_dutycycle", e, reason="d2_raise_preserve")` (the forbidden Reading-B mutation). Assert `test_d2_raise_fusion_byte_identity_reading_a` REDS.
   - **6b (spurious release):** source-mutate the same failure branch to instead call `exclusion_set.release("stuck_dutycycle", e)` for entities in `_prev_excluded`. Assert the recovered-NM-storm regression test AND `test_d2_raise_fusion_byte_identity_reading_a` still hold (this mutation is a semantic no-op under Reading A because `reset_tick()` already cleared the mirror — but is documented as a wire-in probe so a future refactor that changes reset semantics does not silently re-enable Reading-B via the release path). Restore + status-check.
7. **Subscribe-teardown lifecycle wire test (L-LOW-B):** `test_chatter_detector_unsubscribe_called_on_teardown` — DELETE the `self._chatter_unsub()` call in `ChatterDetector.async_teardown()`; assert the lifecycle test reds (the HA-returned unsub Mock records zero calls). Restore + status-check.

Run all drills with `PYTHONDONTWRITEBYTECODE=1` + cleared `__pycache__`.

**Acceptance Criteria — D6:**
- **Test:** `test_chatter_listener_wired`, `test_chatter_tick_site_promotes_via_exclusion_set`, `test_chatter_surface_wired`, `test_chatter_release_wired`, `test_exclusion_set_consumer_sites_wired` (parameterized over 6 sites), `test_d2_raise_fusion_byte_identity_reading_a`, `test_chatter_detector_unsubscribe_called_on_teardown`.
- **Verify (mutation):** `≥12 failed` on mutation (4 chatter + 6 consumer sites + 1 release-scan failure-branch add-promote + 1 subscribe-teardown), `0 failed` on restore.
...
11. **Reading B (D2-raise fusion re-engagement of prev-tick D1 entities).** Explicit non-goal per §D1.1 point 3. Byte-identity to shipped v5.75.0 is load-bearing this cycle; any fusion-preservation-on-detector-raise work is a separate follow-up cycle with its own named acceptance test.
12. **Consuming `OccupancySubstrate.subscribe()` from the chatter client.** Its callback signature is per-KIND aggregate (`(room, kind, on)` at `occupancy_substrate.py:704-706`) and cannot deliver the per-entity edges the chatter burst-counter requires. Chatter owns its own `async_track_state_change_event` listener. A future substrate refactor that adds a per-entity feed could re-open this, but is not in scope.
...
- **Depends on:** for chatter — the detector's OWN `async_track_state_change_event(hass, <blind-time-gated entity set>, self._on_edge)` per-entity HA listener (registered in `ChatterDetector.async_setup`, unsub stored as `self._chatter_unsub`) + `self._entity_to_kind` map (built from `_KIND_TO_CONF` inversion at `occupancy_substrate.py:82-86` plus the opener CONF, walked once at setup) + per-entity last-transition tracker + `_d2_boot_settle_done()` + D0 physics criterion + `CHATTER_PROVENANCE_ALLOWLIST` classifier. For P22 — `_sensor_on_since` + hours threshold. For STUCK-SENSOR-1 — `_detect_duty_cycle_stuck` + role-aware corroborators + `_d2_house_state_allows`.
- **Health of dependencies:** `async_track_state_change_event` is a stable HA core helper (used at `coordinator.py:1350`); `_KIND_TO_CONF` inversion is a one-shot setup-time read of committed constants; boot-settle proven; role-aware corroborators proven (SENSOR-CAPABILITY-1 shipped); D0 grounds the chatter criterion in prior art before ship. Note: chatter does NOT consume `OccupancySubstrate.subscribe()` — its per-KIND aggregate signature cannot deliver per-entity edges (see §6 non-goal 12).
...
| `custom_components/universal_room_automation/domain_coordinators/chatter_detector.py` (new) | `ChatterDetector` — OWNS its `async_track_state_change_event` listener (registered in `async_setup`, unsub stored as `self._chatter_unsub`) + `_entity_to_kind` map built by inverting `_KIND_TO_CONF` + opener CONF at setup + `_on_edge` per-entity handler + physics criterion + release + `async_teardown()` invoking the HA-returned unsub | +260 |
| `custom_components/universal_room_automation/coordinator.py` | Migrate 6 consumer sites onto `SensorExclusionSet.is_excluded()`; migrate P22 + STUCK-SENSOR-1 D1 writers onto `promote()` (lock-step with `_dutycycle_excluded_now` per §D1.1); construct `ChatterDetector` (which registers its OWN `async_track_state_change_event` listener in its `async_setup` — RoomCoordinator does NOT call `substrate.subscribe(...)` on chatter's behalf); store `self._chatter_detector` + call `async_teardown()` from `async_will_remove_from_hass`; tick-site promotion/release block + kill-switch helper; PRESERVE `_d2_completed_cleanly` failure-branch semantics unchanged per §D1.1 Reading A (do NOT re-populate `SensorExclusionSet` mirror on the failure branch) | +200 |
...
| `quality/tests/test_sensor_exclusion.py` (new) | STEP D1 shared-primitive tests (multi-client, byte-identity, scope, mutation) + D1.1 `test_d2_raise_fusion_byte_identity_reading_a` fixture | +260 |
| `quality/tests/test_chatter_detector.py` (new) | D2 definition-agnostic tests + D4 no-PIR test + M-MED-2 provenance-gate fixture tests + L-LOW-B `test_chatter_detector_unsubscribe_called_on_teardown` (spies HA `async_track_state_change_event` return; asserts the HA unsub is called exactly once on teardown); definition-anchored tests added when D0 lands | +260 |
| `quality/tests/test_chatter_release.py` (new) | D3 tests | +140 |
| `quality/tests/test_unavailable_entities_chatter.py` (new) | D5 tests | +80 |
| `quality/tests/test_chatter_wire_in.py` (new) | D6 wire-in + mutation drill (chatter listener + tick-site + surface + release + 6 consumer sites + release-scan failure-branch add-promote + subscribe-teardown) | +240 |
| `docs/planning/RESEARCH_sensor_chatter_definition_prior_art.md` (in flight, separate task) | D0 research doc | — |
| `docs/planning/PROBE_sensor_chatter_definition_handcheck.md` (LANDED, D0 companion) | D0 recorder hand-check (GO-with-amendments) — referenced, not authored by this cycle | — |

**No changes to:** `database.py`, `notification_manager.py`, `occupancy_substrate.py` (strengthened: chatter does not consume `subscribe()` and does not need any new per-entity feed on the substrate — see §6 non-goal 12), `_stuck_signal_nm.py`, `actuator_reconciler.py`. The chatter listener is owned end-to-end by `ChatterDetector`.
...
- **Review A — data integrity + fixture byte-identity + consumer / scope enumeration re-verification.** Owns: independent grep of every `SensorExclusionSet.is_excluded()` consumer to verify all 6 sites migrated (Bug Class #53); STEP-EXCLUDE-4 scope grep (primitive not imported outside RoomCoordinator); `ledger_golden_replay.py` green (STUCK-SENSOR-1 D1 rows AND recovered_stuck_signal rows byte-identical after migration onto `promote()`; chatter's persisted anomaly row shape compatible with MANIFEST "chatter_adjudication" block); the NEW D2-raise byte-identity fixture (`test_d2_raise_fusion_byte_identity_reading_a`) asserts Reading-A preservation on the failure tick; no new DB writer; no unbounded per-entity dict growth on entity removal.
- **Review B — physics correctness + STEP-EXCLUDE-{1,2,3} + INV-CHATTER-{1,3,4} + async / lifecycle / restart semantics + §D1.1 release-scan ordering + listener teardown.** Owns: end-to-end trace of a state-change from the HA event bus → `ChatterDetector._on_edge` (via the detector's own `async_track_state_change_event` registration) → `_entity_to_kind` lookup → detector state → tick-site → `SensorExclusionSet.promote("chatter", ...)` → 6 consumer sites; STEP-EXCLUDE-3 client-isolation MUTATION-verified (delete per-client tracking → per-client-release test reds); boot-settle gate mutation-verified; restart resilience (`_chattering_entities` RAM-cleared on restart; per-day NM latch survives per existing `_stuck_signal_nm`); D3 release symmetry with `check_quarantine_release`; unavailable-state handling on both detection (skip) and release (skip); STEP-EXCLUDE-1 second clause (chatter MUST NOT suppress a non-quarantined sensor's true vote); §D1.1 ordering verified (reset_tick → snapshot → P22 → STUCK-1 → chatter; `_d2_completed_cleanly=False` branch preserves STUCK-1's OWN book only, does NOT re-populate `SensorExclusionSet` mirror — Reading A); listener teardown (L-LOW-B): the `async_track_state_change_event`-returned unsubscribe callable stored on the detector as `self._chatter_unsub`, called from `RoomCoordinator.async_will_remove_from_hass()` via `ChatterDetector.async_teardown()`, no leaked HA listener across a room-entry reload.
- **Review C — surfaces (D5) + test authority via per-site source mutation + wire-in anchors + D0 anchoring + M-MED-2 classifier fixtures.** Owns: D5 attr round-trip through operator dashboard; D6 wire-in tests fail on deletion; mutation-verified with `PYTHONDONTWRITEBYTECODE=1` + cleared `__pycache__`; D0 research doc AND companion probe (`PROBE_sensor_chatter_definition_handcheck.md`) both present in repo; the chatter DEFINITION step in D2 is genuinely built on D0's un-fakeable criterion (not a raw rate re-emerging under a new name); busy-real / chatter-fault fixture pair authored per D0 recipe and each fixture actually discriminates; `CHATTER_PROVENANCE_ALLOWLIST` classifier fixtures (camera-motion group false-positive guard, integration-domain fallback for mislabeled entities, silent-default DENY on unknown families); D6 test 6a (forbidden Reading-B add-promote mutation) reds the D2-raise byte-identity fixture; D6 test 7 (subscribe-teardown wire) reds the lifecycle test on `self._chatter_unsub()` deletion (the HA-returned unsub Mock records zero calls after deletion).
- **Review D — adversarial completeness / diff-blind (Tier 3 addition).** Sole job: state the load-bearing invariant (above) in falsifiable form and BREAK it. Re-enumerate the ENTIRE invariant surface — INCLUDING pre-existing code, not just this cycle's diff (v5.75.0 stuck path, P22 continuous-on, chatter, `SensorExclusionSet` API, all 6 consumer sites, provenance-gate classifier, §D1.1 release-scan, Reading-A byte-identity claim on the D2-raise codepath). For every candidate leak, produce a **concrete, legal-config, reachable repro** (the entity kind + provider + operator-settable knob values + house state that trigger it), not just a code path. Specifically enumerate: (i) any promote/release path that could quarantine a blind-time-gated sensor emitting ZERO sub-`T_floor` events (invariant part 1); (ii) any promote path that misses a sensor with sustained sub-`T_floor` bursts within the observation window (invariant part 2); (iii) any consumer site or substrate path that lets a quarantined vote leak into the fusion, OR drops an untrusted-sensor's fusion contribution beyond the intended room-tier vote (invariant part 3); (iv) the §D1.1 release-scan interaction under partial-detector failure + restart + calendar rollover, INCLUDING any helper path that could smuggle in Reading-B (fusion re-engagement of prev-tick D1) on the failure branch; (v) the M-MED-2 classifier boundary cases (mislabeled `device_class`, missing provider tag, aggregate whose member is chattering, silent-default DENY of a new device family that should have been in-scope); (vi) `CHATTER_BURST_K` / `T_floor` config combinatorics at extremes (K=1, K=0, T_floor=0, T_floor absurdly large, per-sensor T_floor=0 kill-switch interacting with a genuinely-broken sensor); (vii) HA-listener leak surface — is there any RoomCoordinator lifecycle event (reload, entry-migrate, hass-shutdown) that bypasses `async_will_remove_from_hass()` and leaves the `ChatterDetector`'s `async_track_state_change_event` registration active; also (viii) `_entity_to_kind` staleness — if the room's CONF_*_SENSORS list changes at runtime (options-flow edit), does the detector's cached map get refreshed or does an obsolete entity_id still get scored / a new one silently skipped? Run in parallel with A/B/C; D's framing MUST NOT overlap theirs.

**Orchestrator independent verification before ship — MANDATORY, do not trust reviewer summaries.** Personally re-grep every emission site (`promote("chatter"`, `promote("stuck_dutycycle"`, `promote("p22_continuous"`, and all 6 `is_excluded()` consumer sites) AND the `async_track_state_change_event(` registration inside `ChatterDetector` AND re-run at least one per-site source mutation on the load-bearing tick-site loop AND the §D1.1 release-scan failure branch (D6 test 6a — forbidden add-promote) AND the subscribe-teardown deletion (D6 test 7). Confirm named-test failure on mutation and clean restore per `feedback_unrestored_mutation_drill`.

**Operator checkpoint BEFORE deploy** (Tier 3 requirement — not just before build). Surface the final review outcomes from A/B/C/D + the invariant-proof summary + the mutation-drill evidence + the ledger_golden byte-identity result + the D2-raise Reading-A fixture result. Get explicit go before invoking deploy.sh. If D (or any) finds a CRITICAL/HIGH: fix, re-verify the fixed site with its own mutation-anchored test, AND re-run D's completeness enumeration (a fix can reveal an N+1th site).

**Plan review — Tier 3 (2 plan reviews minimum per CLAUDE.md 2026-08-11 rule; the elevation applies to the BUILD review, and the plan itself was reviewed at Tier 2-DB single-pass and returned PLAN-NEEDS-FIXES 3 MED — fixes applied 2026-08-19 — followed by a second plan-review pass (build-prediction framing) which returned PLAN-NEEDS-FIXES 1 MED + 2 LOW — fixes applied 2026-08-19; PLAN-READY for Tier-3 build dispatch under the §9 four-framing-disjoint protocol. Further amended 2026-08-19 to fix the load-bearing `OccupancySubstrate.subscribe()` gap surfaced by the build — see §D2 "Why not `OccupancySubstrate.subscribe()`" and §6 non-goal 12):**
...
- **Plan-Review-2 (adversarial build-prediction):** what will the builder get wrong? Ambiguities in `SensorExclusionSet` API (does promote()-then-promote() by the same client repeat-log or dedupe?); the D2/D3 tick-site ordering vs STUCK-SENSOR-1 D1's existing block (see §D1.1); the `_chattering_entities` vs `SensorExclusionSet.provenance()` two-surface parity trap; classifier fallback ordering (camera-family regex must fire BEFORE motion-provider allow); §D1.1 failure-branch Reading A vs Reading B (M2-MED-1 — resolved: Reading A picked); module vs helper location (L-LOW-A — resolved: module for both D1 and D2); subscribe teardown lifecycle (L-LOW-B — resolved: `self._chatter_unsub` + `async_will_remove_from_hass` + lifecycle test); **feed-shape mismatch (2026-08-19 amendment — resolved: chatter owns its own `async_track_state_change_event` per-entity listener; the `OccupancySubstrate.subscribe()` per-KIND aggregate feed CANNOT drive per-entity sub-`T_floor` burst counting, so the earlier "no new registration / inherit subscribe" framing is VOID)**.
...
- **Reading B (D2-raise fusion re-engagement of prev-tick D1 entities on detector failure)** → parked per §D1.1 point 3 / §6 non-goal 11. Trigger: field evidence of transient-detector-fault-driven vote drops causing operator-visible false-vacant events; would then land as its own cycle with a named "D2-raise fusion preservation" acceptance test.
- **Substrate per-entity feed** → parked per §6 non-goal 12. If future clients also need per-entity edges filtered through substrate provenance, a `subscribe_entities()` API could be added; not required for chatter (owns its own listener).
...
- §D1.1 failure-branch semantics → **Reading A** (bookkeeping-only preservation, byte-identical to shipped v5.75.0). Reading B parked (§10). Resolved 2026-08-19 per plan-review-2 M2-MED-1.
- D1 + D2 code location → **new modules** (`sensor_exclusion.py`, `chatter_detector.py`); alternative helper-in-`coordinator.py` REJECTED. Resolved 2026-08-19 per plan-review-2 L-LOW-A.
- Subscribe teardown → **`self._chatter_unsub` (the callable returned by the detector's OWN `async_track_state_change_event` registration) stored on `ChatterDetector`; teardown from `RoomCoordinator.async_will_remove_from_hass()` via `ChatterDetector.async_teardown()`; lifecycle test `test_chatter_detector_unsubscribe_called_on_teardown` spies the HA unsub Mock and asserts one call on teardown.** Resolved 2026-08-19 per plan-review-2 L-LOW-B and further clarified 2026-08-19 (feed-shape amendment: not the `OccupancySubstrate.subscribe()` unsub, which would have been the wrong feed entirely — see §D2 "Why not `OccupancySubstrate.subscribe()`" and §6 non-goal 12).
- Chatter listener feed → **detector owns its OWN `async_track_state_change_event` per-entity listener** over the room's blind-time-gated entity set (Option 1, orchestrator-chosen 2026-08-19). `OccupancySubstrate.subscribe()` is NOT used — its per-KIND aggregate signature `(room, kind, on)` at `occupancy_substrate.py:704-706` cannot deliver per-entity edges. See §D2 "Why not `OccupancySubstrate.subscribe()`", §7 producer dependencies, and §6 non-goal 12.
