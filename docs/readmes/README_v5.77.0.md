...
| L2 | Census exceeds 4 with gathering present | **ORGANIC (open)** | Post-boot readings: 0 (boot transient) -> 4; no census-camera traversal yet since restart. Counting path proven: detectors read 6 (family room) this afternoon pre-deploy; suffix mapping drill-anchored in-suite. PASS = first recorder reading >4. |
