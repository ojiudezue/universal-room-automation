...
| L3 | **Zone delete works end-to-end on the husk zone `Entertainment + Master Suite`** — every entity/device/DB-row keyed by it goes with it; no zone resurrection after a follow-up restart | **PASS (validated 2026-07-12, with one defect found)** | Operator deleted via UI ~14:29 CDT. Verified on-host: husk gone from ZM `data`+`options`; 0 rows across the 3 name-keyed tables (zone_id purge correctly skipped — husk resolved no zone_id); zone device + all name-keyed entities removed; rooms' stale `CONF_ZONE` strings stripped (3 rooms now on house zone "Entertainment"), so the boot migration (`__init__.py:133-148`) cannot re-mint it. **Post-restart resurrection check (bare restart 19:42 CDT): PASS** — 5 house zones only, no husk, zero URA errors. **DEFECT found (hotfix pending):** the prune signal's name-fallback (`hvac.py:1724`) matched the LIVE merged HVAC zone_1 (Entertainment + Master Suite share `climate.thermostat_bryant_wifi_studyb_zone_1`) and knocked it out of the ZoneManager 14:29→19:42 (~5h inert zone logic; thermostat kept its own program). Recovery on restart was automatic and complete — discovery re-derived zone_1, all 12 zone_id-keyed entities live again 19:43-19:44. |
| L4 | **Exactly ONE reload during delete** — no reload storm across 40 Room entries | **UNVERIFIABLE-LIVE** | The reload/delete INFO lines are hidden by the WARNING-level file logger (`home-assistant.log` not written; docker logs protection-locked). Indirect evidence consistent with single reload: no boot-storm symptoms, no room-entity churn in the recorder at delete time. |
