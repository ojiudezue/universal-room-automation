...
| L4 | Sleep transition works the next guest-straddles-22:00 evening: GUEST exits by gate-clear + ~12 min chain (300s exit persistence + hysteresis) | **PASS — ORGANIC 2026-07-16** | Recorder: `guest` armed 19:19 → resolved `home_evening`→`home_night` 21:03 → `sleep` 22:00 sharp. Pre-fix this latch blocked sleep all night. All 3 post-deploy nights (07-14/15/16) reached sleep at/near 22:00; false-arm guest states now resolve in ~20-90 min instead of latching. |
| L5 | Empty-house: next full-family departure engages AWAY within ~5 min with veto_path=lost_admitted_immediate, NO flap cycle | PENDING-ORGANIC (re-checked 2026-07-16: no qualifying full-family departure window in 3 days; no counter-evidence; one ambiguous away/arriving burst 07-14 19:51-23:17 — unresolved whether real comings/goings, not recurred) | house-state sensor attrs + history |
