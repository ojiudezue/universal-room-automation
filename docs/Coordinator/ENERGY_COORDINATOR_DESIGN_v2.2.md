...
## 10. DIAGNOSTICS, AUDITING & SELF-OPTIMIZATION

### Overview: The Learning Coordinator

Coordinators make decisions based on predictions and rules, but reality intervenes:
- **Humans override** - Someone cranks the AC during peak
- **Predictions miss** - Solar forecast was wrong, occupancy prediction missed a visitor
- **Conditions drift** - House thermal characteristics change seasonally
- **No feedback loop** - Without measurement, we can't improve

This section defines a **reusable framework** for all coordinators to:
1. **Log** every decision with full context
2. **Track** compliance (what actually happened vs what was commanded)
3. **Measure** outcomes (did it work? savings? comfort impact?)
4. **Learn** patterns (when/why do overrides happen?)
5. **Adapt** parameters through Bayesian updating

### Architecture: Observe → Audit → Learn → Adapt
...
│                                                                              │
│  ┌───────────────┐     ┌───────────────┐     ┌───────────────┐              │
│  │ 1. DECISION   │     │ 2. COMPLIANCE │     │ 3. OUTCOME    │              │
│  │    LOGGING    │────▶│    TRACKING   │────▶│  MEASUREMENT  │              │
│  │               │     │               │     │               │              │
│  │ Every action  │     │ Commanded vs  │     │ Savings,      │              │
│  │ with context  │     │ actual state  │     │ comfort, etc. │              │
│  └───────────────┘     └───────────────┘     └───────────────┘              │
│                                                      │                       │
│                                                      ▼                       │
│                              ┌───────────────────────────────────┐          │
│                              │ 4. PATTERN ANALYSIS (Weekly)      │          │
│                              │                                   │          │
│                              │ "User overrides coast 73% when    │          │
│                              │  outdoor temp > 95°F"             │          │
│                              └───────────────────────────────────┘          │
│                                                      │                       │
│                                                      ▼                       │
...
│                              │                                   │          │
│                              │ Bayesian update: coast_offset     │          │
│                              │ 3.0°F → 2.5°F based on overrides  │          │
│                              └───────────────────────────────────┘          │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
Every coordinator decision is logged with full context for later analysis:

```python
# domain_coordinators/diagnostics/decision_log.py
...
class DecisionLog:
    ...
    # Identity
    timestamp: datetime
    coordinator_id: str           # "energy", "hvac", "lighting"
    ...
    context: dict                 # Full EnergyContext or equivalent
    
    # Action taken
    action: dict                  # Full action details
...
Track whether devices actually followed commands:

```python
# domain_coordinators/diagnostics/compliance.py
...
class ComplianceRecord:
    ...
    # What was commanded
    commanded_state: dict         # {mode: "coast", setpoint: 77, ...}
    
    # What actually happened
    actual_state: dict            # {mode: "heat_cool", setpoint: 73, ...}
...
class ComplianceTracker:
    ...
    COMPLIANCE_CHECK_DELAY = 120  # Seconds to wait before checking compliance
    ...
    async def schedule_compliance_check(
        ...
        async def _check():
            ...
            await self._check_compliance(decision_id, device_type, device_id, commanded_state)
    ...
    async def _check_compliance(
        ...
        # Compare
        compliant, deviation = self._compare_states(commanded_state, actual_state, device_type)
        
        # Detect override source
        override_source = None
        if not compliant:
            override_source = await self._detect_override_source(device_id, device_type)
        ...
        # Log to database
        await self._log_compliance(record)
    ...
    async def _detect_override_source(self, device_id: str, device_type: str) -> str:
        """Attempt to detect what caused the override."""
        
        # Check recent state changes in logbook
        # This is heuristic - we look at the context of the change
        
        if device_type == "hvac_zone":
            # Check if thermostat was manually adjusted
            # Carrier Infinity sets preset_mode to "manual" when user touches it
            state = self.hass.states.get(device_id)
            if state and state.attributes.get("preset_mode") == "manual":
                return "thermostat_manual"
        
        # Check if another automation triggered
        # (Would need to inspect context.user_id or similar)
...
Measure actual results after each TOU period:

```python
# domain_coordinators/diagnostics/outcomes.py
...
class OutcomeMeasurement:
    ...
    # Cost outcomes
    actual_cost: float
    baseline_cost: float          # What it would have cost without optimization
    savings: float
    
    # Comfort outcomes
    comfort_violations: int       # Rooms that exceeded bounds
...
class OutcomeMeasurer:
    ...
    async def measure_period_outcome(
        ...
        Called at each TOU transition to measure the just-completed period.
        """
        
        # Get actual energy data from statistics
        ...
        # Baseline: what if we had just imported everything we consumed?
        ...
        # Comfort outcomes
        comfort_violations = await self._count_comfort_violations(period_start, period_end)
        max_deviation, rooms = await self._get_max_temp_deviation(period_start, period_end)
        overrides = await self._count_overrides(period_start, period_end)
        
        # Prediction accuracy
        solar_predicted = predictions.get("solar_kwh", 0)
        solar_error = (solar_kwh - solar_predicted) / solar_predicted if solar_predicted else 0
        
        load_predicted = predictions.get("load_kwh", 0)
        load_actual = total_consumption
        load_error = (load_actual - load_predicted) / load_predicted if load_predicted else 0
        ...
        outcome = OutcomeMeasurement(
            ...
            solar_error_pct=solar_error * 100,
            load_predicted=load_predicted,
            load_actual=load_actual,
            load_error_pct=load_error * 100,
        )
...
Analyze patterns weekly/monthly for insights:

```python
# domain_coordinators/diagnostics/patterns.py
...
class OverridePattern:
    ...
    frequency: float              # 0.0-1.0 (how often this happens)
    confidence: float             # Statistical confidence
    sample_size: int
    recommendation: str | None    # "Reduce coast offset when hot"
...
class PatternAnalyzer:
    ...
    def analyze_override_patterns(self, days: int = 30) -> list[OverridePattern]:
        ...
        - When do users override our decisions?
        - What conditions correlate with overrides?
        - Are certain rooms/devices more likely to be overridden?
        """
        patterns = []
        
        # Get override data with context
        ...
        hot_overrides = [o for o in overrides if o["outdoor_temp"] > 95]
        if hot_overrides:
            freq = len(hot_overrides) / len(overrides)
            if freq > 0.3:  # Significant pattern
                patterns.append(OverridePattern(
                    description=f"User overrides HVAC coast {freq:.0%} of time when outdoor > 95°F",
        ...
        evening_overrides = [o for o in overrides if 17 <= o["hour"] <= 20]
        ...
        room_overrides = {}
        for o in overrides:
            room = o.get("room_id", "unknown")
            room_overrides[room] = room_overrides.get(room, 0) + 1
        
        for room, count in room_overrides.items():
            freq = count / len(overrides)
            if freq > 0.3:
                patterns.append(OverridePattern(
                    description=f"{room} accounts for {freq:.0%} of HVAC overrides",
                    condition=f"room == {room}",
                    frequency=freq,
                    confidence=self._calculate_confidence(count, len(overrides)),
                    sample_size=count,
                    recommendation=f"Consider room-specific settings for {room}",
                ))
        
        # Analyze by day of week (weekend patterns?)
        weekend_overrides = [o for o in overrides if o["weekday"] >= 5]
        if weekend_overrides:
            freq = len(weekend_overrides) / len(overrides)
            expected = 2/7  # Weekends are 2/7 of days
            if freq > expected * 1.5:  # Significantly more weekend overrides
                patterns.append(OverridePattern(
                    description=f"Weekend overrides are {freq/expected:.1f}x higher than expected",
                    condition="weekend",
                    frequency=freq,
                    confidence=self._calculate_confidence(len(weekend_overrides), len(overrides)),
                    sample_size=len(weekend_overrides),
                    recommendation="Consider weekend-specific comfort settings",
                ))
    ...
    def analyze_prediction_drift(self, days: int = 30) -> dict:
        ...
        Returns bias corrections that can be applied to forecasts.
        """
        ...
        solar_errors = [
            ...
            for o in outcomes
            if o["solar_predicted"] > 0
        ]
        
        if solar_errors:
            avg_solar_bias = statistics.mean(solar_errors)
        ...
        load_errors = [
            ...
            for o in outcomes
            if o["load_predicted"] > 0
        ]
        
        if load_errors:
            avg_load_bias = statistics.mean(load_errors)
        ...
        return {
            "solar_forecast_bias": avg_solar_bias,
            "solar_forecast_std": solar_std,
            "solar_correction_multiplier": 1.0 / (1.0 + avg_solar_bias) if avg_solar_bias != -1 else 1.0,
            "load_forecast_bias": avg_load_bias,
            "load_forecast_std": load_std,
            "load_correction_multiplier": 1.0 / (1.0 + avg_load_bias) if avg_load_bias != -1 else 1.0,
            "sample_size": len(outcomes),
        }
    
    def calculate_optimization_effectiveness(self, days: int = 30) -> dict:
        """
        Calculate how effective our optimization strategies are.
        
        Returns metrics on what's working and what isn't.
        """
        outcomes = self._get_outcomes(days)
        ...
        total_savings = sum(o["savings"] for o in outcomes)
        total_baseline = sum(o["baseline_cost"] for o in outcomes)
        
        # Effectiveness by period
        peak_outcomes = [o for o in outcomes if o["tou_period"] == "peak"]
        peak_savings = sum(o["savings"] for o in peak_outcomes) if peak_outcomes else 0
        
        # Comfort cost
        total_violations = sum(o["comfort_violations"] for o in outcomes)
        total_overrides = sum(o["manual_overrides"] for o in outcomes)
        
        return {
            "total_savings": total_savings,
            "savings_per_day": total_savings / days,
            "savings_percentage": total_savings / total_baseline * 100 if total_baseline else 0,
            "peak_savings": peak_savings,
            "peak_export_kwh": sum(o["export_kwh"] for o in peak_outcomes),
            "comfort_violations_total": total_violations,
            "comfort_violations_per_day": total_violations / days,
            "override_rate": total_overrides / len(outcomes) if outcomes else 0,
            "sample_days": days,
        }
```

### 10.5 Bayesian Parameter Learning

Adaptive parameter tuning based on observed data:

```python
# domain_coordinators/diagnostics/bayesian_learning.py
...
class ParameterBelief:
    ...
    def update(self, observation: float, observation_weight: float = 1.0) -> "ParameterBelief":
        """
        Bayesian update with new observation.
        
        Uses simple weighted average update (approximation to proper Bayesian).
        """
        # Weight based on how confident we are in prior vs observation
        prior_weight = 1.0 / (self.std ** 2) if self.std > 0 else 1.0
        obs_weight = observation_weight
        
        total_weight = prior_weight + obs_weight
        
        new_mean = (self.mean * prior_weight + observation * obs_weight) / total_weight
...
class BayesianParameterLearner:
    ...
    This is the "self-optimization" component that allows coordinators
    to improve over time without manual tuning.
    """
    ...
    def __init__(self, coordinator_id: str, db_path: str):
        ...
        # Initialize prior beliefs (starting parameters)
        self.beliefs = {
            # HVAC parameters
            ...
            # Thresholds
            "high_temp_override_threshold": ParameterBelief(mean=95, std=5),
        }
        
        # Load saved beliefs from database
        self._load_beliefs()
    ...
    def update_from_outcomes(self, analysis_days: int = 7) -> dict[str, float]:
        ...
        Called periodically (e.g., weekly) to refine parameters.
        """
        
        updates = {}
        
        # Get pattern analysis
        analyzer = PatternAnalyzer(self.db.path)
        ...
        for pattern in override_patterns:
            if "outdoor" in pattern.condition and pattern.frequency > 0.5:
                # High override rate when hot → reduce coast offset
                current = self.beliefs["coast_setpoint_offset"]
                # Observation: users want smaller offset
                optimal_offset = current.mean * (1 - pattern.frequency * 0.3)
                self.beliefs["coast_setpoint_offset"] = current.update(
                    optimal_offset, 
                    observation_weight=pattern.confidence
                )
                updates["coast_setpoint_offset"] = self.beliefs["coast_setpoint_offset"].mean
        
        # Update based on comfort violations
        if effectiveness.get("comfort_violations_per_day", 0) > 1:
            # Too many violations → reduce aggressiveness
            current = self.beliefs["coast_setpoint_offset"]
            self.beliefs["coast_setpoint_offset"] = current.update(
                current.mean * 0.9,  # 10% reduction
                observation_weight=0.5
            )
            updates["coast_setpoint_offset"] = self.beliefs["coast_setpoint_offset"].mean
        
        # Update battery export threshold based on effectiveness
        if effectiveness.get("peak_export_kwh", 0) < 2:
            # Not exporting enough → lower threshold
        ...
        for param, new_value in updates.items():
            self._log_parameter_change(param, new_value)
    ...
    def get_parameters(self) -> dict[str, float]:
        ...
        Returns the mean of each belief distribution.
        """
        return {
            name: belief.mean
    ...
    def get_parameter_with_exploration(self, name: str) -> float:
        """
        Get parameter value with exploration (sampling from belief).
        
        Occasionally try different values to learn faster.
        Useful for A/B testing parameter values.
        """
        if name not in self.beliefs:
            raise KeyError(f"Unknown parameter: {name}")
        
        # 10% of the time, explore by sampling
    ...
    def _log_parameter_change(self, name: str, new_value: float) -> None:
...
### 10.6 Diagnostic Sensors

Each coordinator exposes diagnostic sensors for visibility:

```yaml
# ENERGY COORDINATOR DIAGNOSTIC SENSORS
...
sensor.energy_coordinator_compliance:
  ...
  attributes:
    ...
    hvac_compliant: false
    pool_compliant: true
    evse_compliant: true
    override_count_today: 3
    override_sources: ["hvac_manual", "hvac_manual", "pool_app"]
    compliance_rate_7day: 0.78

sensor.energy_coordinator_effectiveness:
  state: "good"  # excellent, good, fair, poor
  attributes:
    estimated_savings_today: 2.45
    estimated_savings_month: 68.30
    peak_import_avoided_kwh: 8.3
    peak_export_kwh: 4.1
    comfort_violations_today: 1
    prediction_accuracy_7day: 0.82
...
sensor.energy_coordinator_learning:
  ...
  attributes:
    ...
    top_override_pattern: "HVAC coast when outdoor > 95°F"
    solar_forecast_bias: -0.12
    solar_correction_applied: 0.88
    recommended_changes:
      - "reduce_coast_offset"
      - "extend_pre_cool"
    last_learning_update: "2026-01-20T03:00:00"

# HVAC COORDINATOR DIAGNOSTIC SENSORS

sensor.hvac_coordinator_compliance:
  state: "overridden"
  attributes:
    constraint_mode: "coast"
    constraint_setpoint_offset: 3.0
    zones_compliant: ["zone_2_upstairs"]
    zones_overridden: ["zone_1_master", "zone_3_main"]
    compliance_rate_today: 0.45
    override_details:
      zone_1_master:
...
sensor.hvac_coordinator_effectiveness:
  ...
  attributes:
    ...
    avg_coast_duration_minutes: 25

# Per-zone diagnostics
sensor.hvac_zone_1_diagnostics:
  state: "healthy"
  attributes:
    zone_name: "Master Suite"
    constraint_compliance: false
    current_vs_target_temp_delta: 4.0
    hvac_runtime_during_peak_minutes: 25
    fan_activations: 3
    override_detected: true
    override_duration_minutes: 35
    learned_user_preference:
      max_coast_offset: 1.5
      preferred_sleep_temp: 72
```

### 10.7 Database Schema

```sql
-- SQLite schema for coordinator diagnostics
...
CREATE TABLE IF NOT EXISTS decision_log (
    ...
    constraints_published TEXT,  -- JSON array
    devices_commanded TEXT       -- JSON array
);

CREATE INDEX idx_decision_timestamp ON decision_log(timestamp);
CREATE INDEX idx_decision_coordinator ON decision_log(coordinator_id);
...
CREATE TABLE IF NOT EXISTS compliance_log (
    ...
    commanded_state TEXT NOT NULL,  -- JSON
    actual_state TEXT NOT NULL,     -- JSON
    compliant BOOLEAN NOT NULL,
    deviation_details TEXT,         -- JSON
    override_detected BOOLEAN,
    override_source TEXT,
    override_duration_minutes INTEGER
);

CREATE INDEX idx_compliance_decision ON compliance_log(decision_id);
CREATE INDEX idx_compliance_compliant ON compliance_log(compliant);
...
CREATE TABLE IF NOT EXISTS outcome_log (
    ...
    rooms_exceeded TEXT,            -- JSON array
...
CREATE INDEX idx_outcome_period ON outcome_log(period_start, period_end);

-- Parameter beliefs (Bayesian learning)
...
CREATE INDEX idx_param_history ON parameter_history(coordinator_id, parameter_name);
```
